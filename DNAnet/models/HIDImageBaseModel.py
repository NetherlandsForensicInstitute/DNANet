...
class HIDImageBaseModel(TrainableModel, ABC):
    ...
    def set_up_metrics(self, use_evaluation_metric: bool) -> List[Optional[Metric]]:
        """
        Set up the metrics to compute during training and validation.
        """
        raise NotImplementedError
    ...
    def create_predictions(self, logits: torch.Tensor, batch: Sequence[HIDImage]) -> List[Prediction]:
        ...
        Create `Prediction` instances from the raw output of the model.
         ...
         before being returned as a `Prediction`. For instance, for a
         segmentation model, we might want to apply a sigmoid function to the
         raw output and then convert it to a binary mask before returning it as
         a `Prediction`.
        """
        raise NotImplementedError
    ...
    def update_metric(self,
                      ...
                      y_true: Tensor):
        """
        Update the provided metric with the current batch's logits and true labels.
        This allows for flexibility in how the metric is updated based on the
        raw output of the model and the true labels. For instance, for a
        segmentation model, we might want to apply a sigmoid function to the
        raw output before updating the metric, while for a classification
        model, we might want to apply a softmax function instead.
        """
        raise NotImplementedError
