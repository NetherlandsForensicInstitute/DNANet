...
# Part 2: Generating synthetic EPGs
...
Example (from `synthetic_profiles/generateEPG`):
```bash
python generate.py --csv_dir='../generated/generated_alleles_20251223_191249/epgs' \
  --output_dir='../generated_epgs/1' --batch_size=64 --epg_shape 4000 5000 --std_dev 4
```
This reads the CSV profiles in `--csv_dir`, writes `.npy` EPGs to `--output_dir`, processes in batches of 64, shapes each EPG to 4000x5000, and uses a Gaussian std dev of 4 for the idealized peaks.

`--epg_shape` takes two ints: `scan_min` and `epg_length`. With `--epg_shape 4000 5000`, the code keeps scans from 4000 to 4000+5000 and maps them to a 5000-wide array. Increase `scan_min` to skip early empty baseline; adjust `epg_length` to change the output width if your scan range is shorter/longer.
...
The GAN-based generator is described in:
```bibtex
