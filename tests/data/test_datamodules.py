...
def _make_mock_peaks(n: int = 8) -> list[ExtractedPeak]:
    ...
    return [
        ExtractedPeak(
            ...
            marker_name='D3S1358',
            marker_index=i % 28,
            annotation_idx=0 if labels[i] == 'allele' else 1,
        )
        for i in range(n)
    ]
...
class TestPeakClassificationTransformer:
    ...
    def test_datamodule_batches_without_marker_when_disabled(
        self,
        ppf6c,
        nfi_rnd_dataset,
    ) -> None:
        ...
        # When include_marker=False, inputs is just the peak tensor (not a tuple)
        assert inputs.shape[0] <= 4
        assert inputs.shape[1:] == (1, 120)
...
class TestReconstructionTransformer:
    def test_datamodule_returns_preprocessed_pairs(self) -> None:
        ...
        dataset = SplitPreservingDataset(
            images,
            transform=ReconstructionTransformer(autoencoder_log_scale=True, n_dyes=5),
        )
        dm = DNANetDataModule(dataset, batch_size=4, val_fraction=0.2, seed=42)
        dm.setup('fit')
...
class TestCombinedTransformer:
    ...
    def test_datamodule_returns_nested_peaknet_batch(
        ...
        images = [_make_fake_image(f'combined_{i}.hid', peak_count=3) for i in range(10)]
        dataset = SplitPreservingDataset(
            images,
