...
from tests.conftest import RD_DIR, HID_DIR, ANNOT_DIR, PANEL_PATH
...
class TestParseCalledAlleles:
    ...
    def allele_report(self) -> str:
        return str(ANNOT_DIR / 'Dataset 1 DTH_AlleleReport.txt')
    ...
    def test_second_sample(self, nfi_rnd_kit, nfi_rnd_dataset):
        ...
        report = str(ANNOT_DIR / 'Dataset 1 DTH_AlleleReport.txt')
        dataset_strat = nfi_rnd_dataset
    ...
    def test_ladder_adjusted_scanpoint_annotation_matches_reference_mask(self, nfi_rnd_kit):
        ...
        image = HIDImage(
            path=HID_DIR / '1A2_A01_01.hid',
            scaling_strategy=nfi_rnd_kit,
            load_in_memory=True,
        )
        assert image.data is not None

        allele_report = ANNOT_DIR / 'Dataset 1 DTH_AlleleReport.txt'
