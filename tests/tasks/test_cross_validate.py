...
class TestRun:
    ...
    def test_loads_dataset_from_config_and_delegates(self):
        from dnanet.tasks.cross_validate import run

        cfg = _make_cfg(data={"dataset": {"_target_": "some.Dataset"}})
        dataset = MagicMock(name="dataset")
        expected = {"per_fold": [], "aggregate": {}}

        with (
            patch("dnanet.tasks.cross_validate.L.seed_everything"),
            patch("dnanet.tasks.cross_validate.instantiate", return_value=dataset),
            patch("dnanet.tasks.cross_validate.run_with_data", return_value=expected) as rwm,
        ):
            result = run(cfg)

        rwm.assert_called_once_with(cfg, dataset)
        assert result is expected


class TestRunWithData:
    def _run(self, cfg, folds, trainers, n_instantiate_per_fold=3):
        """Helper: mock all I/O, run run_with_data, return result."""
        from dnanet.tasks.cross_validate import run_with_data
        ...
        with (
            ...
            return run_with_data(cfg, MagicMock())

    def test_trainer_fit_called_once_per_fold(self, tmp_path):
        cfg = _make_cfg(output_dir=str(tmp_path), splitting={"k_folds": 2, "test_fraction": 0.0, "seed": 42})
    ...
    def test_saves_aggregate_metrics_json(self, tmp_path):
        cfg = _make_cfg(output_dir=str(tmp_path), splitting={"k_folds": 1, "test_fraction": 0.0, "seed": 42})
    ...
    def test_keyboard_interrupt_stops_loop_early(self, tmp_path):
        ...
        cfg = _make_cfg(output_dir=str(tmp_path), splitting={"k_folds": 3, "test_fraction": 0.0, "seed": 42})
        ...
        with (
            ...
            result = run_with_data(cfg, MagicMock())
    ...
    def test_skips_scheduler_instantiation_when_not_configured(self, tmp_path):
        ...
        with (
            patch("dnanet.tasks.cross_validate.L.seed_everything"),
            patch("dnanet.tasks.cross_validate.dataset_splitter", return_value=(folds, None)),
            patch("dnanet.tasks.cross_validate.instantiate", side_effect=[_mock_network(), MagicMock(), MagicMock()]) as inst_mock,
            patch("dnanet.tasks.cross_validate.DataLoader", return_value=MagicMock()),
            patch("dnanet.tasks.cross_validate.L.Trainer", return_value=trainer),
            patch("dnanet.tasks.cross_validate._build_callbacks", return_value=[]),
            patch("dnanet.tasks.cross_validate._build_logger", return_value=None),
        ):
            run_with_data(cfg, MagicMock())
    ...
    def test_instantiates_scheduler_when_configured(self, tmp_path):
        ...
        with (
            patch("dnanet.tasks.cross_validate.L.seed_everything"),
            patch("dnanet.tasks.cross_validate.dataset_splitter", return_value=(folds, None)),
            patch("dnanet.tasks.cross_validate.instantiate", side_effect=[_mock_network(), MagicMock(), MagicMock(), MagicMock()]) as inst_mock,
            patch("dnanet.tasks.cross_validate.DataLoader", return_value=MagicMock()),
            patch("dnanet.tasks.cross_validate.L.Trainer", return_value=trainer),
            patch("dnanet.tasks.cross_validate._build_callbacks", return_value=[]),
            patch("dnanet.tasks.cross_validate._build_logger", return_value=None),
    ...
    def test_metric_tensors_converted_to_float(self, tmp_path):
        ...
        tensor_val.item.return_value = 0.75
        trainer = _make_trainer({"val/loss": tensor_val})

        with (
            patch("dnanet.tasks.cross_validate.L.seed_everything"),
            patch("dnanet.tasks.cross_validate.dataset_splitter", return_value=(folds, None)),
            patch("dnanet.tasks.cross_validate.instantiate", side_effect=[_mock_network(), MagicMock(), MagicMock()]),
            patch("dnanet.tasks.cross_validate.DataLoader", return_value=MagicMock()),
            patch("dnanet.tasks.cross_validate.L.Trainer", return_value=trainer),
            patch("dnanet.tasks.cross_validate._build_callbacks", return_value=[]),
            patch("dnanet.tasks.cross_validate._build_logger", return_value=None),
        ...
        assert result["per_fold"][0]["val/loss"] == 0.75
