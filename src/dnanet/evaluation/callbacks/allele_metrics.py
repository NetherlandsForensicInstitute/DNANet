...
class AlleleMetricsCallback(Callback):
    """Compute allele-level metrics during Lightning validation and test runs."""
    ...
    def on_validation_epoch_start(self, trainer: L.Trainer, pl_module: L.LightningModule) -> None:
        """Reset allele metrics before a validation epoch starts."""
    ...
    def on_validation_batch_end(
        ...
        """Update allele metrics from validation batch predictions and metadata."""
        if getattr(trainer, 'sanity_checking', False):
            return
        self._update_from_outputs(outputs, batch, batch_idx, trainer, pl_module, dataloader_idx)

    def on_validation_epoch_end(self, trainer: L.Trainer, pl_module: L.LightningModule) -> None:
        """Log allele metrics at validation epoch end."""
        if getattr(trainer, 'sanity_checking', False):
            return
        self._log_stage_metrics('val', trainer, pl_module)
    ...
    def on_test_epoch_end(self, trainer: L.Trainer, pl_module: L.LightningModule) -> None:
        ...
        self._log_stage_metrics('test', trainer, pl_module)
    ...
    def _update_from_outputs(
        ...
        if outputs is None or "preds" not in outputs:
            raise ValueError(
                "AlleleMetricsCallback requires validation_step/test_step to return "
                "a mapping with a 'preds' tensor."
            )
    ...
    def _log_stage_metrics(
        ...
        pl_module.log(f"{stage}/allele_precision", self.precision.compute(), logger=True, sync_dist=True)
        pl_module.log(f"{stage}/allele_recall", self.recall.compute(), logger=True, sync_dist=True)
        pl_module.log(f"{stage}/allele_f1", self.f1.compute(), logger=True, sync_dist=True)
        self._reset_metrics()
